...
>
> **2026-08-04 补充：** 基于对已落地实现的审查（H-01 / M-01~06 / L-01），完成一轮安全与正确性修复（未知异常不泄露、模型上下文不含逻辑路径、输出预算本地强制、health 能力收紧为 `read_file`、前端 SSE 持续订阅与 ready 门禁、失败 UI 错误码映射、路由归属校验、SSE 连接上限原子化）。本文件描述的工作包结构不变；修复细节以 `docs/phase2-model-provider-design.md` 与 `AGENTS.md` 状态追踪为权威。
