...
在 `awaiting_approval` 之前只允许只读工具。在 `applying_change` 之前需要特定的已批准变更集。在发送到 UI 之前持久化每个状态转换和工具结果。
...
包管理器尚未决定。在框架初始化期间选择 `pnpm` 或 `npm` 并记录该决策。
...
- 绑定授权工作区。
- 添加工作区守卫和只读工具。
- 实现状态机、差异审查、审批、受控写入、命令白名单和会话持久化。
